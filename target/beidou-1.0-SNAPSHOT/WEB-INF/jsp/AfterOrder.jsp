        ...
        <div class="layui-side-scroll">
            ...
            <ul class="layui-nav layui-nav-tree"  lay-filter="test">
                <c:if test="${userSession.companyId==1||userSession.companyId==2||userSession.companyId==3}">
                    <li class="layui-nav-item" ><a href="${pageContext.request.contextPath}/user/main">首页</a></li>
                    ...
                    <li class="layui-nav-item layui-nav-itemed">
					    ...
					    box-shadow: #cde2fa;" href="${pageContext.request.contextPath}/Recycle/goLook"> &nbsp;&nbsp;售后订单管理</a></dd>
                ...
                <c:if test="${userSession.companyId==4}">
                    ...
                    <li class="layui-nav-item"><a style="background-color: #4bafff;border-radius: 0 5px 5px 0;box-shadow: #cde2fa;" href="${pageContext.request.contextPath}/Recycle/goLookMe">售后订单管理</a></li>
                </c:if>
            </ul>
        </div>
    </div>
...
<%--                        <span>订单号：<input type="text" class="search" id="orderId" name="orderId" <c:if test="${sessionList.orderId!=null}">value="${sessionList.orderId}" </c:if> > </span>--%>
...
<%--                    </p>--%>
<%--                    <p style="line-height: 30px">--%>
<%--                        <span>收货人：<input type="text" class="search" id="consigName" name="consigName" <c:if test="${sessionList.consigName!=null}">value="${sessionList.consigName}" </c:if>> </span>--%>
...
<%--                    </c:forEach>--%>
<%--                </select>--%>
<%--                <input name="cha" type="button" class="layui-btn layui-btn-sm" onclick="okc()" value="点击查询">--%>
<%--                </span>--%>
...
<script type="text/javascript">
    ...
    function isAudit(obj) {
        layer.open({
            ...
            ,yes: function(index, layero){
                ...
                $.get("${pageContext.request.contextPath}/Recycle/approve?orderId="+theId,function() {

                    layer.open({
                        content: '审核成功，重新发送物流信息'
                        ,btn: ['确定']
                        ,yes: function(index, layero){
                            //按钮【按钮一】的回调
                            window.location.href="${pageContext.request.contextPath}/wuLiu/goAdd";
                        }
                    });
    ...
    function isRefuse(obj) {
        layer.open({
            ...
            ,yes: function(index, layero){
                //按钮【按钮一】的回调
                var tem = obj;
                var orderId = $(tem).attr("orderId");
                window.location.href="${pageContext.request.contextPath}/Recycle/appRefuse?orderId="+orderId;
            }
        });
    }
    var count = 0;//查询数量
    ...
    function getData(curr,limit) {
        ...
        $.ajax({
            type: "POST",
            url: "${pageContext.request.contextPath}/Recycle/goList",
            dataType: "json",
            data: {
                orderId: orderId,
                orderTime: orderTime,
            ...
            success: function(result) {
                ...
                $.each(list,function(index,item){
                    ...
                    html+='<td>'+item.recycleContent+'</td>';
                    html+='<td><a orderId=\''+item.orderId+'\' class="panel-box-edit" onclick="isAudit(this)" title=\'审批\'>审批</a> &nbsp;&nbsp;';
                    html+='<a orderId=\''+item.orderId+'\' class="panel-box-edit" onclick="isRefuse(this)"  title=\'拒绝\'>拒绝</a></td></tr>';
                })
                $('#tableList').html(html);
